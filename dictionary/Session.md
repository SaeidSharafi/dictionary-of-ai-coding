...
این حافظه با نشست تمام می‌شود. نشست تازه از هیچ شروع می‌شود: عاملی که پایان نشست دیروز پایگاه کد شما را خوب می‌شناخت، امروز صبح هیچ‌کدامش را نمی‌شناسد. آنچه می‌ماند [سیستم فایل](./Filesystem.md) است — فایل‌هایی که در یک نشست نوشته می‌شوند را نشست بعدی می‌تواند ب‌خواند، و [انتقال‌های زمینه](./Handoff.md)، [سیستم‌های حافظه](./Memory%20system.md) و [AGENTS.md](./AGENTS.md.md) به همین تکیه می‌کنند.

این شما هستید که انتخاب می‌کنید نشست کجا تمام شود. هر چیزی در نشست بر هر [نوبت](./Turn.md) بعدی اثر می‌گذارد، پس کار‌های نامرتبطی که در یک نشست انجام می‌شوند باقی‌مانده‌ای بر جا می‌گذارند که پاسخ بعدی را رنگ می‌دهد. یک کار در هر نشست زمینه را مرتبط نگه می‌دارد؛ تمام کردن یک کار، نقطه طبیعی پاک‌سازی است.
